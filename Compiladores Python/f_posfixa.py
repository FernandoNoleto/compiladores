class Posfixa(object):

    #Função que verifica se a expressão é válida
    def expressao_valida(self, expressao):
        i = 0
        try:
            if (expressao.index('\\')):
                j = 0
    ...
    def operador_valido(self, operador):
    ...
    def prioridade(self, operador_atual, operador2):
    ...
    '''
    #expressão que converte a expressão perfeita para pósfixa
    def polonesa_reversa(self, expressao):
        expressao_reversa = '' #String para formar a pósfixa
        i = 0
        ...
        while (i < len(expressao)):
            if(expressao[i] == '\\'):
                ...
                #expressao_reversa += expressao[i]
                    continue
            if(self.operador_valido(expressao[i])):
                if(expressao[i] == ')'):
                    #expressao_reversa += pilha[-1]
                    while(pilha[-1] != '('):
                        expressao_reversa += pilha[-1]
                    ...
                    continue
                if(pilha):
                    if(self.prioridade(expressao[i],pilha[-1])):
                        expressao_reversa += pilha.pop()
                    if(pilha):
                        if(self.prioridade(expressao[i],pilha[-1])):
                            expressao_reversa += pilha.pop()
    ...
    def expressao_perfeita(self, expressao):
        i = 0
        aux = ''  #string que vai se concatenada com o decorrer do laço para colocar a cocatenação implicíta.
        j = 0
        ultimo_simbolo = expressao[-1]
